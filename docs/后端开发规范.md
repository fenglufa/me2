# 后端开发规范

## 📋 目录

- [1. 项目结构规范](#1-项目结构规范)
  - [1.1 服务目录结构](#11-服务目录结构)
  - [1.2 模块化架构规范](#12-模块化架构规范)
  - [1.3 公共库目录](#13-公共库目录)
- [2. 代码规范](#2-代码规范)
- [3. go-zero 服务开发规范](#3-go-zero-服务开发规范)
- [4. 数据库规范](#4-数据库规范)
- [5. API 设计规范](#5-api-设计规范)
- [6. 错误处理规范](#6-错误处理规范)
- [7. 日志规范](#7-日志规范)
- [8. 测试规范](#8-测试规范)
- [9. 安全规范](#9-安全规范)
- [10. 性能优化规范](#10-性能优化规范)
- [11. 代码审查清单](#11-代码审查清单)
- [12. 常用命令](#12-常用命令)
- [13. Makefile 管理规范](#13-makefile-管理规范)
- [14. 参考资源](#14-参考资源)

---

## 1. 项目结构规范

### 1.1 服务目录结构

每个 go-zero 服务遵循统一的目录结构：

```
services/
└── {service-name}/
    ├── api/                    # API 服务 (可选)
    │   ├── {service}.api      # API 定义文件
    │   ├── {service}.go       # 入口文件
    │   └── internal/
    │       ├── config/        # 配置
    │       ├── handler/       # HTTP 处理器
    │       ├── logic/         # 业务逻辑
    │       ├── svc/           # 服务上下文
    │       └── types/         # 类型定义
    │
    ├── rpc/                    # RPC 服务 (主要)
    │   ├── {service}.proto    # Proto 定义
    │   ├── {service}.go       # 入口文件
    │   └── internal/
    │       ├── config/        # 配置
    │       ├── logic/         # 业务逻辑
    │       ├── server/        # gRPC 服务器
    │       ├── svc/           # 服务上下文
    │       └── model/         # 数据模型
    │
    ├── model/                  # 数据模型 (共享)
    │   ├── {table}model.go    # 表模型
    │   └── vars.go            # 常量定义
    │
    └── README.md              # 服务说明文档
```

### 1.2 模块化架构规范

本项目采用 **Polyrepo** 模式，每个服务都是独立的 Go 模块。

#### 架构原则

- ✅ **服务独立**: 每个服务有独立的 `go.mod`，可以单独开发、测试、部署
- ✅ **依赖隔离**: 每个服务只包含自己需要的依赖包
- ✅ **团队协作**: 不同服务可以由不同团队负责，甚至独立的代码仓库
- ✅ **公共复用**: `pkg` 作为独立模块，通过 Go module 统一管理

#### 完整目录结构

```
backend/
├── pkg/                    # 公共库 (独立模块)
│   ├── go.mod             # module: github.com/me2/pkg
│   ├── README.md
│   ├── errcode/
│   ├── response/
│   └── middleware/
│
└── services/              # 微服务目录
    ├── sms/               # 短信服务 (独立模块)
    │   ├── go.mod         # module: github.com/me2/sms
    │   ├── Makefile
    │   ├── README.md
    │   └── rpc/
    │
    └── oss/               # OSS 服务 (独立模块)
        ├── go.mod         # module: github.com/me2/oss
        ├── Makefile
        ├── README.md
        └── rpc/
```

#### go.mod 管理规范

**pkg 模块**:
```go
// backend/pkg/go.mod
module github.com/me2/pkg

go 1.21

require (
    github.com/zeromicro/go-zero v1.6.1
)
```

**服务模块** (以 sms 为例):
```go
// backend/services/sms/go.mod
module github.com/me2/sms

go 1.21

require (
    github.com/me2/pkg v0.0.1  // 引用公共库
    github.com/zeromicro/go-zero v1.6.1
    // ... 其他依赖
)

// 本地开发时使用 replace
replace github.com/me2/pkg => ../../pkg
```

#### Import 路径规范

**✅ 推荐 - 使用模块路径**:
```go
// 服务内部引用
import (
    "github.com/me2/sms/rpc/internal/config"
    "github.com/me2/sms/rpc/internal/svc"
    "github.com/me2/sms/rpc/sms"
)

// 引用公共库
import (
    "github.com/me2/pkg/errcode"
    "github.com/me2/pkg/response"
)

// 服务间调用
import (
    "github.com/me2/sms/rpc/sms"
)
```

**❌ 不推荐 - 使用相对路径**:
```go
import (
    "me2/backend/services/sms/rpc/internal/config"  // 错误
    "../config"                                      // 错误
)
```

#### 依赖初始化流程

```bash
# 初始化 pkg 模块
cd backend/pkg
go mod tidy

# 初始化服务模块
cd backend/services/sms
go mod tidy

# 或使用 Makefile 批量初始化
cd backend
make init-all
```

### 1.3 公共库目录

```
pkg/
├── llmclient/              # LLM 客户端封装
├── vectorclient/           # 向量数据库客户端
├── ossclient/              # OSS 客户端
├── smsclient/              # 短信客户端
├── middleware/             # 中间件
│   ├── auth.go            # 认证中间件
│   ├── logger.go          # 日志中间件
│   └── recovery.go        # 恢复中间件
├── utils/                  # 工具函数
│   ├── crypto.go          # 加密工具
│   ├── time.go            # 时间工具
│   └── validator.go       # 验证工具
├── errcode/                # 错误码定义
│   └── code.go
└── response/               # 统一响应
    └── response.go
```

---

## 2. 代码规范

### 2.1 命名规范

#### 文件命名
- 使用小写字母和下划线: `user_service.go`
- 测试文件: `user_service_test.go`
- 模型文件: `{table}model.go`

#### 变量命名
```go
// ✅ 推荐
var userID int64
var userName string
var isActive bool

// ❌ 不推荐
var user_id int64
var UserName string
var is_active bool
```

#### 常量命名
```go
// ✅ 推荐 - 使用驼峰或全大写
const (
    MaxRetryCount = 3
    DefaultTimeout = 30 * time.Second
)

const (
    STATUS_ACTIVE   = 1
    STATUS_INACTIVE = 0
)
```

#### 函数命名
```go
// ✅ 推荐 - 动词开头，驼峰命名
func GetUserByID(id int64) (*User, error)
func CreateAvatar(req *CreateAvatarRequest) error
func UpdatePersonality(avatarID int64, personality *Personality) error

// ❌ 不推荐
func user_by_id(id int64) (*User, error)
func avatar_create(req *CreateAvatarRequest) error
```

#### 接口命名
```go
// ✅ 推荐 - 以 er 结尾或描述性名词
type UserRepository interface {
    FindByID(id int64) (*User, error)
    Create(user *User) error
}

type AvatarService interface {
    GetPersonality(avatarID int64) (*Personality, error)
}
```

### 2.2 注释规范

#### 包注释
```go
// Package userservice 提供用户账号管理功能
// 包括用户注册、登录、订阅管理等核心功能
package userservice
```

#### 函数注释
```go
// GetUserByPhone 根据手机号查询用户
// 参数:
//   - phone: 手机号码
// 返回:
//   - *User: 用户对象
//   - error: 错误信息，用户不存在时返回 ErrUserNotFound
func GetUserByPhone(phone string) (*User, error) {
    // ...
}
```

#### 复杂逻辑注释
```go
// 计算分身行动意图
// 1. 收集时间段权重
// 2. 计算人格匹配度
// 3. 应用用户互动因子
// 4. 添加随机噪声
score := timeWeight*0.3 + personalityWeight*0.4 +
         userInteractionWeight*0.2 + randomNoise*0.1
```

### 2.3 代码格式

#### 导入顺序
```go
import (
    // 标准库
    "context"
    "fmt"
    "time"

    // 第三方库
    "github.com/zeromicro/go-zero/core/logx"
    "github.com/zeromicro/go-zero/core/stores/sqlx"

    // 项目内部
    "me2/pkg/errcode"
    "me2/services/user/model"
)
```

#### 函数长度
- 单个函数不超过 80 行
- 超过则拆分为多个小函数

#### 文件长度
- 单个代码文件不超过 500 行
- 超过则拆分为多个文件或模块

#### 行宽
- 每行代码不超过 120 字符
- 超过则换行

---

## 3. go-zero 服务开发规范

### 3.1 API 定义规范

```api
syntax = "v1"

info(
    title: "用户服务 API"
    desc: "用户注册、登录、信息管理"
    author: "AI Avatar Team"
    version: "v1.0"
)

type (
    // 登录请求
    LoginRequest {
        Phone string `json:"phone" validate:"required,phone"`
        Code  string `json:"code" validate:"required,len=6"`
    }

    // 登录响应
    LoginResponse {
        Token    string `json:"token"`
        UserID   int64  `json:"user_id"`
        AvatarID int64  `json:"avatar_id,omitempty"`
    }
)

@server(
    group: auth
    prefix: /api/v1/auth
)
service user {
    @doc "用户登录"
    @handler login
    post /login (LoginRequest) returns (LoginResponse)
}
```

### 3.2 Proto 定义规范

```protobuf
syntax = "proto3";

package user;
option go_package = "./user";

// 用户服务
service User {
  // 根据 ID 获取用户
  rpc GetUserByID(GetUserByIDRequest) returns (GetUserByIDResponse);

  // 创建用户
  rpc CreateUser(CreateUserRequest) returns (CreateUserResponse);
}

// 请求消息
message GetUserByIDRequest {
  int64 user_id = 1;
}

// 响应消息
message GetUserByIDResponse {
  int64 user_id = 1;
  string phone = 2;
  int32 subscription_tier = 3;
  int64 created_at = 4;
}
```

### 3.3 配置文件规范

```yaml
Name: user-rpc
ListenOn: 0.0.0.0:8080

# 数据库配置
DB:
  DataSource: postgres://user:pass@localhost:5432/me2?sslmode=disable
  MaxOpenConns: 100
  MaxIdleConns: 10
  MaxLifetime: 3600

# Redis 配置
Redis:
  Host: localhost:6379
  Type: node
  Pass: ""

# 日志配置
Log:
  ServiceName: user-rpc
  Mode: console
  Level: info
  Encoding: json

# 链路追踪
Telemetry:
  Name: user-rpc
  Endpoint: http://localhost:14268/api/traces
  Sampler: 1.0
  Batcher: jaeger
```

### 3.4 Logic 层规范

```go
type GetUserLogic struct {
    ctx    context.Context
    svcCtx *svc.ServiceContext
    logx.Logger
}

func NewGetUserLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetUserLogic {
    return &GetUserLogic{
        ctx:    ctx,
        svcCtx: svcCtx,
        Logger: logx.WithContext(ctx),
    }
}

// GetUser 获取用户信息
// 业务逻辑:
// 1. 参数验证
// 2. 查询数据库
// 3. 返回结果
func (l *GetUserLogic) GetUser(in *user.GetUserRequest) (*user.GetUserResponse, error) {
    // 1. 参数验证
    if in.UserId <= 0 {
        return nil, errcode.ErrInvalidParam
    }

    // 2. 查询数据库
    userModel, err := l.svcCtx.UserModel.FindOne(l.ctx, in.UserId)
    if err != nil {
        if err == model.ErrNotFound {
            return nil, errcode.ErrUserNotFound
        }
        l.Errorf("查询用户失败: %v", err)
        return nil, errcode.ErrInternalServer
    }

    // 3. 返回结果
    return &user.GetUserResponse{
        UserId: userModel.Id,
        Phone:  userModel.Phone,
    }, nil
}
```

---

## 4. 数据库规范

### 4.1 表命名规范

- 使用小写字母和下划线
- 使用复数形式: `users`, `avatars`, `events_history`
- 关联表使用两个表名: `avatar_locations`

### 4.2 字段命名规范

```sql
-- ✅ 推荐
CREATE TABLE users (
    id BIGSERIAL PRIMARY KEY,
    phone VARCHAR(20) NOT NULL UNIQUE,
    subscription_tier INT DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- ❌ 不推荐
CREATE TABLE Users (
    ID BIGSERIAL PRIMARY KEY,
    Phone VARCHAR(20),
    SubscriptionTier INT
);
```

### 4.3 索引规范

```sql
-- 主键索引
PRIMARY KEY (id)

-- 唯一索引
CREATE UNIQUE INDEX idx_users_phone ON users(phone);

-- 普通索引
CREATE INDEX idx_avatars_user_id ON avatars(user_id);

-- 复合索引 (最常查询的字段在前)
CREATE INDEX idx_events_avatar_time ON events_history(avatar_id, occurred_at DESC);
```

### 4.4 Model 层规范

```go
package model

import (
    "context"
    "database/sql"
    "github.com/zeromicro/go-zero/core/stores/sqlx"
)

var _ UserModel = (*customUserModel)(nil)

type (
    // UserModel 用户模型接口
    UserModel interface {
        userModel
        // 自定义方法
        FindByPhone(ctx context.Context, phone string) (*User, error)
        UpdateSubscription(ctx context.Context, userID int64, tier int) error
    }

    customUserModel struct {
        *defaultUserModel
    }
)

// NewUserModel 创建用户模型
func NewUserModel(conn sqlx.SqlConn) UserModel {
    return &customUserModel{
        defaultUserModel: newUserModel(conn),
    }
}

// FindByPhone 根据手机号查询用户
func (m *customUserModel) FindByPhone(ctx context.Context, phone string) (*User, error) {
    query := `SELECT * FROM users WHERE phone = $1 LIMIT 1`
    var user User
    err := m.conn.QueryRowCtx(ctx, &user, query, phone)
    if err != nil {
        if err == sql.ErrNoRows {
            return nil, ErrNotFound
        }
        return nil, err
    }
    return &user, nil
}
```

---

## 5. API 设计规范

### 5.1 RESTful 规范

```
GET    /api/v1/users/:id          # 获取用户
POST   /api/v1/users               # 创建用户
PUT    /api/v1/users/:id          # 更新用户
DELETE /api/v1/users/:id          # 删除用户

GET    /api/v1/avatars/:id/personality    # 获取性格
PATCH  /api/v1/avatars/:id/personality    # 更新性格
```

### 5.2 统一响应格式

```go
// 成功响应
{
    "code": 0,
    "msg": "success",
    "data": {
        "user_id": 123,
        "phone": "13800138000"
    }
}

// 错误响应
{
    "code": 10001,
    "msg": "用户不存在",
    "data": null
}
```

### 5.3 响应封装

```go
package response

type Response struct {
    Code int         `json:"code"`
    Msg  string      `json:"msg"`
    Data interface{} `json:"data,omitempty"`
}

func Success(data interface{}) *Response {
    return &Response{
        Code: 0,
        Msg:  "success",
        Data: data,
    }
}

func Error(code int, msg string) *Response {
    return &Response{
        Code: code,
        Msg:  msg,
        Data: nil,
    }
}
```

### 5.4 分页规范

```go
type PageRequest struct {
    Page     int `json:"page" form:"page" validate:"min=1"`
    PageSize int `json:"page_size" form:"page_size" validate:"min=1,max=100"`
}

type PageResponse struct {
    Total    int64       `json:"total"`
    Page     int         `json:"page"`
    PageSize int         `json:"page_size"`
    List     interface{} `json:"list"`
}
```

---

## 6. 错误处理规范

### 6.1 错误码定义

```go
package errcode

var (
    // 通用错误 (1xxxx)
    ErrInvalidParam    = NewError(10001, "参数错误")
    ErrInternalServer  = NewError(10002, "服务器内部错误")
    ErrUnauthorized    = NewError(10003, "未授权")

    // 用户相关 (2xxxx)
    ErrUserNotFound    = NewError(20001, "用户不存在")
    ErrUserExists      = NewError(20002, "用户已存在")
    ErrInvalidCode     = NewError(20003, "验证码错误")

    // 分身相关 (3xxxx)
    ErrAvatarNotFound  = NewError(30001, "分身不存在")
    ErrAvatarExists    = NewError(30002, "分身已存在")

    // AI 相关 (4xxxx)
    ErrAIServiceFailed = NewError(40001, "AI 服务调用失败")
    ErrPromptNotFound  = NewError(40002, "Prompt 模板不存在")
)

type Error struct {
    Code int
    Msg  string
}

func NewError(code int, msg string) *Error {
    return &Error{Code: code, Msg: msg}
}

func (e *Error) Error() string {
    return e.Msg
}
```

### 6.2 错误处理模式

```go
// ✅ 推荐 - 明确的错误处理
user, err := l.svcCtx.UserModel.FindOne(l.ctx, userID)
if err != nil {
    if err == model.ErrNotFound {
        return nil, errcode.ErrUserNotFound
    }
    l.Errorf("查询用户失败: userID=%d, err=%v", userID, err)
    return nil, errcode.ErrInternalServer
}

// ❌ 不推荐 - 忽略错误
user, _ := l.svcCtx.UserModel.FindOne(l.ctx, userID)
```

### 6.3 错误日志

```go
// 记录错误上下文
l.Errorf("创建分身失败: userID=%d, name=%s, err=%v",
    req.UserId, req.Name, err)

// 记录堆栈信息 (严重错误)
l.Errorw("AI 服务调用失败",
    logx.Field("user_id", userID),
    logx.Field("prompt_id", promptID),
    logx.Field("error", err),
    logx.Field("stack", string(debug.Stack())),
)
```

---

## 7. 日志规范

### 7.1 日志级别

```go
// Debug - 调试信息
l.Debugf("查询参数: userID=%d, page=%d", userID, page)

// Info - 一般信息
l.Infof("用户登录成功: userID=%d, phone=%s", userID, phone)

// Warn - 警告信息
l.Warnf("验证码即将过期: phone=%s, expireAt=%v", phone, expireAt)

// Error - 错误信息
l.Errorf("数据库查询失败: err=%v", err)
```

### 7.2 结构化日志

```go
// ✅ 推荐 - 使用结构化日志
l.Infow("AI 调用成功",
    logx.Field("service", "event"),
    logx.Field("avatar_id", avatarID),
    logx.Field("template_id", templateID),
    logx.Field("tokens", tokens),
    logx.Field("duration_ms", duration),
)

// ❌ 不推荐 - 字符串拼接
l.Infof("AI 调用成功: service=event, avatar_id=%d, tokens=%d", avatarID, tokens)
```

### 7.3 敏感信息脱敏

```go
// ✅ 推荐 - 脱敏处理
phone := "13800138000"
maskedPhone := phone[:3] + "****" + phone[7:]
l.Infof("发送验证码: phone=%s", maskedPhone) // 138****8000

// ❌ 不推荐 - 直接记录敏感信息
l.Infof("发送验证码: phone=%s", phone)
```

---

## 8. 测试规范

### 8.1 单元测试

```go
package logic

import (
    "context"
    "testing"
    "github.com/stretchr/testify/assert"
)

func TestGetUserLogic_GetUser(t *testing.T) {
    // 准备测试数据
    ctx := context.Background()
    svcCtx := &svc.ServiceContext{
        // Mock 依赖
    }

    logic := NewGetUserLogic(ctx, svcCtx)

    // 测试用例
    tests := []struct {
        name    string
        userID  int64
        want    *user.GetUserResponse
        wantErr bool
    }{
        {
            name:   "正常获取用户",
            userID: 1,
            want: &user.GetUserResponse{
                UserId: 1,
                Phone:  "13800138000",
            },
            wantErr: false,
        },
        {
            name:    "用户不存在",
            userID:  999,
            want:    nil,
            wantErr: true,
        },
    }

    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got, err := logic.GetUser(&user.GetUserRequest{
                UserId: tt.userID,
            })

            if tt.wantErr {
                assert.Error(t, err)
            } else {
                assert.NoError(t, err)
                assert.Equal(t, tt.want, got)
            }
        })
    }
}
```

### 8.2 测试覆盖率

```bash
# 运行测试并生成覆盖率报告
go test -v -cover ./...

# 生成 HTML 覆盖率报告
go test -coverprofile=coverage.out ./...
go tool cover -html=coverage.out -o coverage.html

# 要求: 核心业务逻辑覆盖率 > 80%
```

### 8.3 基准测试

```go
func BenchmarkGetUser(b *testing.B) {
    ctx := context.Background()
    svcCtx := &svc.ServiceContext{}
    logic := NewGetUserLogic(ctx, svcCtx)

    b.ResetTimer()
    for i := 0; i < b.N; i++ {
        logic.GetUser(&user.GetUserRequest{UserId: 1})
    }
}
```

---

## 9. 安全规范

### 9.1 SQL 注入防护

```go
// ✅ 推荐 - 使用参数化查询
query := `SELECT * FROM users WHERE phone = $1`
err := m.conn.QueryRowCtx(ctx, &user, query, phone)

// ❌ 不推荐 - 字符串拼接
query := fmt.Sprintf("SELECT * FROM users WHERE phone = '%s'", phone)
```

### 9.2 密码处理

```go
import "golang.org/x/crypto/bcrypt"

// 密码加密
func HashPassword(password string) (string, error) {
    bytes, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost)
    return string(bytes), err
}

// 密码验证
func CheckPassword(password, hash string) bool {
    err := bcrypt.CompareHashAndPassword([]byte(hash), []byte(password))
    return err == nil
}
```

### 9.3 JWT 认证

```go
// 生成 Token
func GenerateToken(userID int64, secret string) (string, error) {
    claims := jwt.MapClaims{
        "user_id": userID,
        "exp":     time.Now().Add(7 * 24 * time.Hour).Unix(),
    }
    token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
    return token.SignedString([]byte(secret))
}

// 验证 Token
func ParseToken(tokenString, secret string) (int64, error) {
    token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
        return []byte(secret), nil
    })
    if err != nil || !token.Valid {
        return 0, errcode.ErrUnauthorized
    }

    claims := token.Claims.(jwt.MapClaims)
    userID := int64(claims["user_id"].(float64))
    return userID, nil
}
```

### 9.4 敏感数据加密

```go
// 使用 AES 加密敏感数据
import "crypto/aes"

// 不要在代码中硬编码密钥
// 从环境变量或配置中心读取
encryptionKey := os.Getenv("ENCRYPTION_KEY")
```

---

## 10. 性能优化规范

### 10.1 数据库优化

```go
// ✅ 推荐 - 批量查询
userIDs := []int64{1, 2, 3, 4, 5}
users, err := l.svcCtx.UserModel.FindByIDs(ctx, userIDs)

// ❌ 不推荐 - 循环查询 (N+1 问题)
for _, id := range userIDs {
    user, _ := l.svcCtx.UserModel.FindOne(ctx, id)
}
```

### 10.2 缓存使用

```go
// 查询时先查缓存
cacheKey := fmt.Sprintf("user:%d", userID)
cached, err := l.svcCtx.Redis.Get(cacheKey)
if err == nil {
    var user User
    json.Unmarshal([]byte(cached), &user)
    return &user, nil
}

// 缓存未命中，查询数据库
user, err := l.svcCtx.UserModel.FindOne(ctx, userID)
if err != nil {
    return nil, err
}

// 写入缓存 (设置过期时间)
data, _ := json.Marshal(user)
l.svcCtx.Redis.Setex(cacheKey, string(data), 3600)
```

### 10.3 并发控制

```go
// 使用 sync.WaitGroup 并发处理
var wg sync.WaitGroup
results := make(chan *Result, len(avatarIDs))

for _, avatarID := range avatarIDs {
    wg.Add(1)
    go func(id int64) {
        defer wg.Done()
        result := processAvatar(id)
        results <- result
    }(avatarID)
}

wg.Wait()
close(results)
```

### 10.4 连接池配置

```yaml
DB:
  MaxOpenConns: 100      # 最大打开连接数
  MaxIdleConns: 10       # 最大空闲连接数
  MaxLifetime: 3600      # 连接最大生命周期(秒)

Redis:
  PoolSize: 100          # 连接池大小
  MinIdleConns: 10       # 最小空闲连接数
```

---

## 11. 代码审查清单

提交代码前请确认：

### 代码质量
- [ ] 代码符合命名规范
- [ ] 添加了必要的注释
- [ ] 错误处理完整
- [ ] 添加了日志记录
- [ ] 单文件不超过 500 行
- [ ] 单函数不超过 80 行

### 模块化架构
- [ ] 使用正确的模块路径 (github.com/me2/...)
- [ ] import 路径使用模块路径，不使用相对路径
- [ ] go.mod 文件正确配置
- [ ] 本地开发使用 replace 指向 pkg
- [ ] 运行 `go mod tidy` 清理依赖

### 测试
- [ ] 编写了单元测试
- [ ] 测试覆盖率 > 80%
- [ ] 运行 `make test` 通过

### 代码检查
- [ ] 运行 `gofmt` 格式化代码
- [ ] 运行 `golint` 检查代码
- [ ] 运行 `go vet` 检查问题

### 安全性
- [ ] 没有硬编码敏感信息
- [ ] SQL 使用参数化查询
- [ ] 密码使用 bcrypt 加密
- [ ] Token 使用 JWT

### 数据库
- [ ] 添加了必要的索引
- [ ] 表名和字段名使用小写+下划线

### 性能优化
- [ ] 考虑了并发安全
- [ ] 考虑了性能优化
- [ ] 避免 N+1 查询问题

---

## 12. 常用命令

```bash
# ========== 模块管理 ==========
# 初始化所有服务依赖
make init-all

# 初始化单个服务
cd services/sms && make init

# 清理并更新依赖
go mod tidy

# 查看依赖
go list -m all

# ========== 代码质量 ==========
# 格式化代码
gofmt -w .

# 代码检查
golint ./...
go vet ./...

# ========== 测试 ==========
# 运行测试
go test -v ./...

# 生成覆盖率报告
go test -coverprofile=coverage.out ./...
go tool cover -html=coverage.out

# ========== RPC 代码生成 ==========
# 生成所有服务的 RPC 代码
make gen-all

# 生成单个服务的 API 代码
goctl api go -api user.api -dir .

# 生成单个服务的 RPC 代码
goctl rpc protoc user.proto --go_out=. --go-grpc_out=. --zrpc_out=.

# ========== Model 代码生成 ==========
# 生成 Model 代码
goctl model pg datasource -url="postgres://user:pass@localhost:5432/me2" -table="users" -dir="./model"

# ========== 服务管理 ==========
# 编译所有服务
make build-all

# 运行服务
cd services/sms && make run

# 清理构建文件
make clean-all
```

---

## 13. Makefile 管理规范

### 13.1 Makefile 结构

每个服务根目录下应包含 Makefile 文件，统一管理常用命令。

```makefile
# 服务名称
SERVICE_NAME := user-rpc
API_FILE := user.api
PROTO_FILE := user.proto

# 默认目标
.PHONY: help
help:
	@echo "可用命令:"
	@echo "  make init        - 初始化依赖"
	@echo "  make build       - 编译服务"
	@echo "  make run         - 运行服务"
	@echo "  make test        - 运行测试"
	@echo "  make cover       - 生成覆盖率报告"
	@echo "  make lint        - 代码检查"
	@echo "  make fmt         - 格式化代码"
	@echo "  make gen-api     - 生成 API 代码"
	@echo "  make gen-rpc     - 生成 RPC 代码"
	@echo "  make gen-model   - 生成 Model 代码"
	@echo "  make clean       - 清理构建文件"

# 初始化依赖
.PHONY: init
init:
	@echo "初始化依赖..."
	@go mod tidy
	@go mod download

# 编译
.PHONY: build
build:
	@echo "编译 $(SERVICE_NAME)..."
	@go build -o bin/$(SERVICE_NAME) .

# 运行
.PHONY: run
run:
	@echo "运行 $(SERVICE_NAME)..."
	@go run .

# 测试
.PHONY: test
test:
	@echo "运行测试..."
	@go test -v ./...

# 覆盖率
.PHONY: cover
cover:
	@echo "生成覆盖率报告..."
	@go test -coverprofile=coverage.out ./...
	@go tool cover -html=coverage.out -o coverage.html
	@echo "覆盖率报告已生成: coverage.html"

# 代码检查
.PHONY: lint
lint:
	@echo "代码检查..."
	@golint ./...
	@go vet ./...

# 格式化
.PHONY: fmt
fmt:
	@echo "格式化代码..."
	@gofmt -w .

# 生成 API 代码
.PHONY: gen-api
gen-api:
	@echo "生成 API 代码..."
	@goctl api go -api $(API_FILE) -dir .

# 生成 RPC 代码
.PHONY: gen-rpc
gen-rpc:
	@echo "生成 RPC 代码..."
	@goctl rpc protoc $(PROTO_FILE) --go_out=. --go-grpc_out=. --zrpc_out=.

# 生成 Model 代码
.PHONY: gen-model
gen-model:
	@echo "生成 Model 代码..."
	@goctl model pg datasource -url="$(DB_URL)" -table="$(TABLE)" -dir="./model"

# 清理
.PHONY: clean
clean:
	@echo "清理构建文件..."
	@rm -rf bin/
	@rm -f coverage.out coverage.html
```

### 13.2 项目根目录 Makefile

项目根目录的 Makefile 用于管理所有服务：

```makefile
# 项目名称
PROJECT_NAME := me2

# 服务列表
SERVICES := user avatar ai event action world dialogue diary memory imagegen notify sms oss

.PHONY: help
help:
	@echo "可用命令:"
	@echo "  make init-all    - 初始化所有服务依赖"
	@echo "  make build-all   - 编译所有服务"
	@echo "  make test-all    - 测试所有服务"
	@echo "  make fmt-all     - 格式化所有代码"
	@echo "  make lint-all    - 检查所有代码"
	@echo "  make clean-all   - 清理所有构建文件"
	@echo "  make gen-all     - 生成所有 RPC 代码"
	@echo "  make docker-up   - 启动 Docker 环境"
	@echo "  make docker-down - 停止 Docker 环境"

# 初始化所有服务依赖
.PHONY: init-all
init-all:
	@echo "初始化 pkg 依赖..."
	@cd pkg && go mod tidy && go mod download
	@for service in $(SERVICES); do \
		echo "初始化 $$service 依赖..."; \
		cd services/$$service && make init && cd ../..; \
	done

# 编译所有服务
.PHONY: build-all
build-all:
	@for service in $(SERVICES); do \
		echo "编译 $$service..."; \
		cd services/$$service && make build && cd ../..; \
	done

# 测试所有服务
.PHONY: test-all
test-all:
	@for service in $(SERVICES); do \
		echo "测试 $$service..."; \
		cd services/$$service && make test && cd ../..; \
	done

# 格式化所有代码
.PHONY: fmt-all
fmt-all:
	@echo "格式化所有代码..."
	@gofmt -w .

# 检查所有代码
.PHONY: lint-all
lint-all:
	@echo "检查所有代码..."
	@golint ./...
	@go vet ./...

# 清理所有构建文件
.PHONY: clean-all
clean-all:
	@for service in $(SERVICES); do \
		echo "清理 $$service..."; \
		cd services/$$service && make clean && cd ../..; \
	done

# Docker 环境
.PHONY: docker-up
docker-up:
	@echo "启动 Docker 环境..."
	@docker-compose up -d

.PHONY: docker-down
docker-down:
	@echo "停止 Docker 环境..."
	@docker-compose down
```

### 13.3 使用规范

#### 命名规范
- 目标名使用小写字母和连字符: `build`, `test`, `gen-api`
- 所有目标都应声明为 `.PHONY`

#### 必需目标
每个服务的 Makefile 必须包含以下目标：
- `help` - 显示帮助信息
- `init` - 初始化依赖 (go mod tidy)
- `build` - 编译服务
- `run` - 运行服务
- `test` - 运行测试
- `fmt` - 格式化代码
- `lint` - 代码检查
- `clean` - 清理构建文件

#### 可选目标
根据服务类型添加：
- `gen-api` - API 服务需要
- `gen-rpc` - RPC 服务需要
- `gen-model` - 有数据库的服务需要
- `cover` - 需要覆盖率报告时添加

#### 环境变量
敏感信息通过环境变量传递：
```makefile
DB_URL ?= $(shell echo $$DB_URL)
API_KEY ?= $(shell echo $$API_KEY)
```

---

## 14. 参考资源

- [Go 官方代码规范](https://golang.org/doc/effective_go)
- [go-zero 官方文档](https://go-zero.dev/)
- [Uber Go 代码规范](https://github.com/uber-go/guide)
- [Google Go 代码规范](https://google.github.io/styleguide/go/)

---

**文档版本**: v1.0
**更新时间**: 2025-12-02
**维护者**: AI Avatar Team
