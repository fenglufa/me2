 🎭 Me2 AI分身系统 - 服务关系与流程详解
  ...
  3️⃣ 【Action Service 计算意图】
     ...
     │
     ├─ 调用 AI Service 计算意图得分
     │  AI → CalculateActionIntent(personality, time_context)
     │  返回各行为得分:
     │  {
     │    exploration: 75,  ← 得分最高
     │    social: 68,
     ...
     │  World → RecommendScene(action_type: "exploration")
     │  返回: {scene_id: 1, scene_name: "星巴克咖啡厅"}
     │
     ├─ 记录行动日志到 action_logs 表
     │
     └─ 返回: {action_type: "exploration", scene_id: 1}
     │
     ▼
  ...
  用户创建分身 → Scheduler 自动调度 → Action 决定行为 → 记录日志

  场景 2: 完整的事件生成流程（已实现但未集成）

  Scheduler 触发 → Action 选择行为 → Event 生成故事 → 用户查看时间线
  ...
  ⏸ 待集成:
  - Action Service 调用 Event Service 生成事件
  - Event 对人格特征的影响反馈
  - 用户查看分身事件时间线的前端

  🎯 核心价值:
  整个系统实现了一个"自主的 AI 分身"，它有自己的性格，会根据性格做出不同的行为选择，并在虚拟世界中"生活"，产生各种有趣的故事。