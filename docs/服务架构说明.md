...
5. **Action + Event Service** (核心玩法 - 行动逻辑 + 事件生成)
6. **Diary Service** (日记系统 - 重要的情感功能)
7. **Dialogue Service** (用户交互)
8. **Memory Service** (长期记忆 - RAG)
9. **ImageGen + Notify** (体验完善)
...
**更新时间:** 2025-12-02
**变更:**
- 新增 AI Service (AI 统一服务) - 所有 AI 调用的统一入口 ⭐
- 集成 Deepseek API 作为统一大模型服务
- 所有服务通过 AI Service 调用 AI 能力,不再直接调用外部 AI API
- 新增 Prompt 模板管理系统和 AI 调用监控
- 新增 Diary Service (日记服务)
- 明确 User 和 Avatar 的职责划分(User不存储昵称头像)
